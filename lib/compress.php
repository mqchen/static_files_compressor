...
class SFCompress {
	...
	protected function processParams(array $params) {
		...
		if(isset($params['cachetimeout'])) {
			$this->cacheTimeout = intval($params['cachetimeout']);
		}
		...
		for($i = 0, $l = count($files); $i < $l; $i++) {
			if(strpos($files[$i], ':') === false) {
				...
				$this->files[] = array('remote', $files[$i]);
				$this->remoteFiles++;
			}
		}
	}
	...
	protected function makeHeaders() {
		...
		//$this->addHeader('Last-modified', date('r', filemtime($this->getCacheFilePath())), false);
	}
...
$s->process();
//readfile($s->getCacheFilePath()); // TODO: refactor to method of SFCompress with gzip, etc.